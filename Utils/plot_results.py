...
def main():
    ...
    ap.add_argument("--nodes", nargs="*", default=None, help="nodes name, e.g. A B1")
    ...
    ap.add_argument("--links", nargs="*", default=None,
                    help="links endpoints, e.g. --link B1 A B1 B2")
    ...
    ap.add_argument("--paper1", action="store_true", help="save paper figures to this path (png/pdf)")
    ap.add_argument("--paper2", action="store_true", help="save paper figures to this path (png/pdf)")
    ap.add_argument("--mixed", action="store_true", help="mix node and link data")
    args = ap.parse_args()
...
def Several_Keys(args):
    ...
    if args.node:
        t, series = load_node_series(args.dir, args.node, args.paper1)
        if args.paper1:
            keys = [r'$H_{'+args.node+'}$']
        ...
        ys1 = {k: series[k] for k in keys if k in series}
        title = f"{model_name} | Node {args.node}"
        if not args.mixed :
            plot_multi_series(t, ys1, title=title, ylabel="value (nats)", save=args.save, show=args.show)

    u, v = args.link
    t, series = load_link_series(args.dir, u, v, args.paper1)
    if args.paper1:
    ...
    keys = args.keys or list(series.keys())
    ys2 = {k: series[k] for k in keys if k in series}
    title = f"{model_name} | Link {u}–{v}"
    if args.mixed :
        for k in ys1:
            ys2[k] = ys1[k]
        title = f"{model_name} | Node {args.node} and Link {u}–{v}"
    plot_multi_series(t, ys2, title=title, ylabel="value (nats)", save=args.save, show=args.show)


def Several_Region(args):
    props = read_properties(os.path.join(args.dir, "Simulation_Properties.txt"))
    model_name = props.get("Model_Name", "Simulation results")

    if not args.node and not args.nodes and not args.link and not args.links:
        raise SystemExit("Provide --node NODE or --link U V.")

    if args.nodes:
        ys = {}
        for node in args.nodes:
            t, series = load_node_series(args.dir, node, True)
            keys = r'$H_{'+node+'}$'
            ys[keys] = series[keys]
        title = f"Figure 3 : {model_name} | Node {args.node}"
    ...
    if args.links:
        ...
        for i in range(int(len(links)/2)):
            u = links[2*i]
            v = links[2*i+1]
            keys_Dict = {"MI":r'$I_{'+u+':'+v+'}$', "TE1":r'$T_{'+v+r' \to '+u+'}$', "rTE1":r'$rT_{'+v+r' \to '+u+'}$', "TE2":r'$T_{'+u+r' \to '+v+'}$', "rTE2":r'$rT_{'+u+r' \to '+v+'}$'}
            keys = keys_Dict[args.keys[0]]
            t, series = load_link_series(args.dir, u, v, True)
